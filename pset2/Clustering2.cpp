...
bool cmp(const PII & b, const int a)
{
	return b.second < a;
}
...
{	
	...
	FOR(i,0,sz(g))
	{
		if (i < (sz(g)-1) && g[i].second == g[i+1].second)
		{
			v.Union(g[i].first, g[i+1].first);
		}
		p = g[i].second;
		FOR(m,0,b)
		{
			...
			{
				key = p^((1<<m) | (1<<n));
				it = lower_bound(g.begin(), g.end(), key, cmp);
				if ((*it).second == key)
				{
					v.Union(g[i].first, (*it).first);
				}
			}
		}
	}
